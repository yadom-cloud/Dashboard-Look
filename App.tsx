...
import {
  Search, Target, AlertTriangle, AlertCircle, Calendar, Database, Copy, X
} from 'lucide-react';

// --- Data Mappers (your original logic, cleaned up) ---
...
const App: React.FC = () => {
  ...
  // --- Helpers (your original getDevLoad) ---
  ...
  // --- Derived State & Sorting (your original) ---
  ...
  // --- Warning Banner Logic (your original) ---
  ...
  // --- Data Loading (your original, with aggressive logging) ---
  const fetchAllData = useCallback(async () => {
    ...
    try {
      ...
      console.log('Dev data raw:', devData);
      console.log('Dev error:', devError);
      if (devError) throw devError;
      if (!devData || devData.length === 0) {
        throw new Error('No developers found in table. Check Supabase data.');
      }
      ...
      setDevLookup(lookup);
      console.log(`✅ Loaded ${mappedDevs.length} developers`);
      ...
      // 3. Availability
      console.log('🛡️ Fetching availability...');
      const { data: blockData, error: blockError } = await supabase.from('manual_availability').select('*');
      console.log('Block data raw:', blockData);
      console.log('Block error:', blockError);
      if (blockError) throw blockError;
      if (blockData) {
        setAvailabilityBlocks(blockData.map(mapBlock));
        console.log(`✅ Loaded ${blockData.length} availability blocks`);
      }
  ...
  // --- Interactions (your original) ---
  const handleTicketDrop = async (ticketId: string, targetDevId: string, targetDate: string) => {
    // Optimistic Update
    setTickets(prev => prev.map(t => {
      if (t.id === ticketId) {
        return { ...t, assigneeId: targetDevId, startDate: targetDate, endDate: targetDate };
      }
      return t;
    }));
  };
...
`;
  ...
  return (
    <div className="flex flex-col h-screen bg-[#f8fafc] font-sans text-[#1e293b] overflow-hidden">
      {/* --- Fixed Top Bar --- */}
      <header className="flex-shrink-0 bg-white border-b border-[#e2e8f0] h-[72px] px-6 flex items-center justify-between z-30 shadow-sm">
        <div className="flex items-center gap-8">
          <div className="flex flex-col justify-center">
            <h1 className="text-[20px] font-semibold text-[#1e293b] leading-none mb-1">Resource Allocation</h1>
      ...
      <div className="flex-grow flex flex-col min-h-0 relative">
        {dataError && (
          <div className="absolute top-4 left-1/2 transform -translate-x-1/2 z-40 bg-red-50 border border-red-200 text-red-700 px-4 py-2 rounded shadow-md flex items-center gap-2">
            ...
            <button onClick={fetchAllData} className="ml-2 text-red-600 hover:underline">Retry</button>
          </div>
        )}
          ...
          {isLoading && (
            <div className="flex items-center justify-center h-full">
              <div className="text-center">
                ...
                <p className="text-gray-600">Loading resource allocation data...</p>
              </div>
            </div>
          )}
          {/* EMPTY STATE */}
          {!isLoading && !dataError && developers.length === 0 && (
            <div className="flex items-center justify-center h-full">
              <div className="text-center">
                <Users className="w-16 h-16 text-gray-300 mx-auto mb-4"/>
                <h3 className="text-lg font-medium text-gray-900 mb-2">No developers to display</h3>
                <p className="text-gray-500 mb-4">Your Supabase "developers" table is empty. Add data to see the dashboard.</p>
                <button onClick={fetchAllData} className="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700">
                  Refresh Data
                </button>
              </div>
            </div>
